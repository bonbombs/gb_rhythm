...
; And a small contribution by Matt Hart

; VERSIONS
; 1.02 02/10/08
;   modified grammar and position of the displayed text
; 1.01 02/09/07
...
; view the upper left corner of the 'canvas'.
	...
	ld	a,0			; SET SCREEN TO TO UPPER LEFT HAND CORNER
	ld	[rSCX], a
	ld	[rSCY], a
...
;Matt's contribution - Jumpy text
	ld	a, $FF		;starts a count down to zero
delay:	
	dec a
	jp nz, delay	;loops until zero

; Your basic copy-paste only I moved the text to the right
; by changing a number.
	...
	call	mem_SetVRAM
	ld	hl,Title
	ld	de, _SCRN0+6+(SCRN_VY_B*7) ; I changed a number 
	ld	bc, TitleEnd-Title
	call	mem_CopyVRAM
	jp center_text	;keeps going back and forth
...
; ****************************************************************************************
Title:
	DB	"Hello, World!!" ;Yep, I changed the grammar of the displayed text
TitleEnd:
